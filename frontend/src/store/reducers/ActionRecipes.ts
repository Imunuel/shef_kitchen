...
import { IDetailRecipe, ISearchRecipes, IShortRecipes, IMenuRecipes, IProfile } from "../../models/models";
...
export const FetchProfileRecipes = (username: string) => async (dispatch: AppDispatch) => {
    try{
        dispatch(profileRecipesSlice.actions.profileRecipesFething())
        const response = await axios.get<IShortRecipes[]>(`recipes/get_my_recipes/?username=${username}`)
        dispatch(profileRecipesSlice.actions.profileRecipesFethingSuccess(response.data))
    }
    catch(e){
        dispatch(profileRecipesSlice.actions.profileRecipesFethingError("ошибка"))
    }
}


export const FetchProfile = (username: string) => async (dispatch: AppDispatch) => {
    try{
        dispatch(profileSlice.actions.profileFething())
        const response = await axios.get<IProfile>(`users/get_me/?username=${username}`)
...
export const CreateRecipe = (name: string, photo: FileList | null, description: string, step_texts: List, step_files: List) => async (dispatch: AppDispatch) => {
    try{
