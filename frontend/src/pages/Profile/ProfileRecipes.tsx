...
import { DeleteProfileRecipes, FetchProfileRecipes } from "../../store/reducers/ActionRecipes";

export const ProfileRecipes: React.FC = ({}) => {

    const dispatch = useAppDispatch()

    const {profile_recipes, isLoading, error} = useAppSelector(state => state.ProfileRecipesReducer)
    useEffect(()=>{
...
<div className="recipes">
  ...
  <div className="recipe" key={recipe.id}>
    ...
    <button className="btn btn-secondary delete_btn" onClick={DeleteRecipe.bind(this, recipe.id)}></button>
  </div>
  )}
  </div>
        </>
